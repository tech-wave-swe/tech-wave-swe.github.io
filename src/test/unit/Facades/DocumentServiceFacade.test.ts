...
jest.mock("vscode", () => ({
  ...
  RelativePattern: jest
    .fn()
    .mockImplementation((folder, pattern) => ({ folder, pattern })),
  window: {
...
const mockDocumentFormatterService = {
  formatSourceCode: jest.fn((code: string, filePath: string): Chunk[] => [
    dummyChunk,
  ]),
...
const mockIVectorDatabase = {
  fileExists:
    jest.fn<(filePath: string, checksum?: string) => Promise<boolean>>(),
...
describe("DocumentServiceFacade", () => {
  ...
  describe("processFiles", () => {
    it("should skip already indexed files", async () => {
      ...
      jest
        .spyOn(FileSystemService, "getChecksum")
        .mockReturnValue("existingChecksum");
      ...
      expect(mockIVectorDatabase.fileExists).toHaveBeenCalledWith(
        "file1.ts",
        "existingChecksum",
      );
      ...
      expect(
        mockDocumentFormatterService.formatSourceCode,
    ...
    it("should process a valid file", async () => {
      ...
      expect(
        mockDocumentFormatterService.formatSourceCode,
      ).toHaveBeenCalledWith("const a = 1;", "file2.ts");
  ...
  describe("processWorkspaceFiles", () => {
    it("should throw if no workspace folder is open", async () => {
      ...
      await expect(facade.processWorkspaceFiles()).rejects.toThrow(
    ...
    it("should warn and return if no matching files are found", async () => {
      (vscode as any).workspace.workspaceFolders = [
        { uri: { fsPath: "workspacePath" }, name: "workspaceFolder" },
      ];
