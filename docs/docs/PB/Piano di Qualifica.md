...
| ---------- | -------- | -------------------------------------------------------- | --------------------- | ------------- | --------------------- |
| 24/04/2025 | 1.8.2    | Commento alle metriche allo sprint 10                    | Dal Bianco Riccardo   | 24/04/2025    | Marcon Giulia         |
...
| 17/04/2025 | 1.7.7    | Fix link tabelle                                         | Vasquez Manuel Felipe | 17/04/2025    | Carraro Agnese        |
| 16/04/2025 | 1.7.6    | Aggiornamento test di sistema                            | Carraro Agnese        | 17/04/2025    | Vasquez Manuel Felipe |
...
| 31/01/2025 | 1.6.0    | Inserimento cruscotto                    | Pistori Gaia          | 05/02/2025    | Vasquez Manuel Felipe |
| 24/01/2025 | 1.5.0    | Aggiunti test                            | Marcon Giulia         | 26/01/2025    | Pistori Gaia          |
| 16/01/2025 | 1.4.0    | Aggiunte misure                          | Monetti Luca          | 17/01/2025    | Vasquez Manuel Felipe |
| 03/01/2025 | 1.3.0    | Modifica descrizione e riferimenti       | Vasquez Manuel Felipe | 06/01/2025    | Monetti Luca          |
| 22/12/2024 | 1.2.0    | Aggiunte metriche di qualità di Prodotto | Pistori Gaia          | 24/12/2024    | Vasquez Manuel Felipe |
| 16/12/2024 | 1.1.0    | Aggiunta qualità di processo             | Marcon Giulia         | 17/12/2024    | Carraro Agnese        |
| 08/12/2024 | 1.0.0    | Prima stesura del documento              | Vasquez Manuel Felipe | 10/12/2024    | Monetti Luca          |
...
- **Norme di Progetto**: [Norme di Progetto - v1.8.5](https://tech-wave-swe.github.io/docs/PB/norme_di_progetto)
...
| Metrica | Nome         | Descrizione                             | Obiettivo                     | Valore Accettabile | Valore Desiderabile |
| ------- | ------------ | --------------------------------------- | ----------------------------- | ------------------ | ------------------- |
| MPRP01  | Adattabilità | Facilità nel setup di un nuovo progetto | Facilitare creazione progetto | ≤ 10m              | ≤ 5min              |
...
Table: Test di unità sulla classe _LanceDBAdapter_ prima parte
...
Table: Test di unità sul file _chat.js_ prima parte
...
Table: Test di unità sul file _chat.js_ seconda parte
...
Table: Test di unità sul file _chat.js_ terza parte
...
Table: Test di unità sul file _track.js_ prima parte
...
Table: Test di unità sul file _track.js_ seconda parte
...
Table: Test di unità sul file _track.js_ terza parte
...
Table: Test di unità sul file _track.js_ sesta parte
...
| TU240 | Verifica che nel file _track.js_, la funzione _escapeHtml_ sanifichi correttamente l'input HTML                                            | Superato |
...
Table: Test di unità sul file _track.js_ settima parte
...
Table: Test di unità sul file _track.js_ ottava parte
...
Table: Test di unità sul file _track.js_ undicesima parte
...
Table: Test di unità sulla classe _ParsingService_ seconda parte
...
| TU503 | Verifica, sulla classe _ChatWebView_, che la funzione _getHtmlForWebview_ gestisca correttamente i casi di errore durante la lettura del file HTML | Superato |
...
| TU509 | Verifica, sulla classe _TrackerWebView_, che la funzione _getHtmlForWebview_ gestisca correttamente i casi di errore durante la lettura del file HTML | Superato |
...
Table: Test di integrazione prima parte
...
| TI19 | Verifica che l'estensione sia presente e attivata correttamente                                                      | Superato |
| TI20 | Verifica che i comandi siano registrati correttamente                                                                | Superato |
| TI21 | Verifica che i provider webview siano registrati correttamente                                                       | Superato |

Table: Test di integrazione seconda parte

| ID   | Descrizione                                                                                        | Stato    |
| ---- | -------------------------------------------------------------------------------------------------- | -------- |
| TI22 | Verifica che il servizio di configurazione sia inizializzato correttamente                         | Superato |
| TI23 | Verifica che i gestori degli eventi siano registrati correttamente                                 | Superato |
| TI24 | Verifica che i servizi di sistema siano inizializzati correttamente                                | Superato |
| TI25 | Verifica che il sistema importi correttamente i requisiti da CSV                                   | Superato |
| TI26 | Verifica che il sistema elabori e indicizzi correttamente i file sorgente                          | Superato |
| TI27 | Verifica che il sistema tracci correttamente l'implementazione dei requisiti                       | Superato |
| TI28 | Verifica che il sistema aggiorni correttamente lo stato dei requisiti                              | Superato |
| TI29 | Verifica che il sistema recuperi correttamente un requisito specifico tramite ID                   | Superato |
| TI30 | Verifica che il sistema esegua correttamente la ricerca vettoriale sui blocchi di codice           | Superato |
| TI31 | Verifica che il sistema analizzi correttamente l'implementazione in base ai riferimenti nel codice | Superato |
| TI32 | Verifica che il sistema pulisca correttamente tutti i requisiti                                    | Superato |

Table: Test di integrazione terza parte
...
Table: Test di sistema prima parte
...
Table: Test di sistema seconda parte
...
Table: Test di sistema quarta parte
...
Table: Test di sistema quinta parte
...
Table: Test di sistema sesta parte
...
Table: Test di sistema settima parte
...
Table: Test di accettazione prima parte
...
Table: Test di accettazione seconda parte
...
Possiamo osservare che abbiamo mantenuto i documenti ben sopra la soglia di accettazione minima, garantendo quindi che siano comprensibili ad utenti con una licenza media.
...
Possiamo osservare che l'utilizzo di "SpellCheck" è risultato fondamentale per minimizzare gli errori ortografici all'interno della documentazione. Questo ci ha permesso di individuarli tempestivamente già al momento della redazione.
...
Possiamo osservare che l'andamento del grafico negli ultimi %%sprint|sprint%% ci conferma il miglioramento nella pianificazione, in quanto il costo effettivo segue l'andamento della spesa preventivata.
...
Possiamo osservare che durante questi ultimi %%sprint|sprint%% abbiamo mantenuto costante il nostro impegno. Infatti, in ciascun periodo siamo riusciti a completare tutte le attività preventivate, riuscendo anche ad anticipare diversi compiti.
...
Possiamo osservare che durante gli ultimi %%sprint|sprint%% siamo riusciti a mantenere costante il nostro livello di impegno. Questo è stato possibile grazie alla conclusione della sessione invernale e delle lezioni permettendo così al gruppo di dedicare più tempo per il progetto.
...
Possiamo osservare che il gruppo è stato in grado di prevedere la maggior parte dei rischi. I rischi inattesi sono stati causati da impegni personali o mancanza di conoscenze tecniche che hanno portato a sottostimare alcune attività.
...
Possiamo osservare che il gruppo si è impegnato sin da subito nell'implementazione dei requisiti obbligatori al fine di soddisfare le aspettative del proponente. Nelle revisioni al codice del penultimo %%sprint|sprint%% è stato raggiunto il completamento.
...
Possiamo osservare che il gruppo ha sviluppato la maggior parte dei requisiti desiderabili concentrandosi su quelli ritenuti maggiormente interessanti da parte dell'azienda proponente e del gruppo.
...
Possiamo osservare che il gruppo ha sviluppato la maggior parte dei requisiti opzionali concentrandosi su quelli ritenuti maggiormente interessanti da parte dell'azienda proponente e del gruppo.
...
Possiamo osservare che il gruppo ha cercato sin da subito di ottenere il livello massimo di branch coverage nel codice, raggiungendo l'obiettivo alla consegna.
...
Possiamo osservare che il gruppo ha cercato sin da subito di ottenere il livello massimo di statement coverage nel codice, raggiungendo l'obiettivo alla consegna.
...
Possiamo osservare che il gruppo ha cercato sin da subito di ottenere il livello massimo di condition coverage nel codice, raggiungendo l'obiettivo alla consegna.

## Metriche non inscluse nel cruscotto

Le seguenti metriche, pur essendo parte integrante della valutazione della qualità del prodotto, non sono state incluse nel cruscotto di valutazione della qualità. Non si prestano infatti ad una rappresentazione grafica.
Riportiamo di seguito i dettagli relativi a ciascuna metrica.

| Metrica | Nome              | Valore Accettabile | Valore Desiderabile | Valore Misurato |
| ------- | ----------------- | ------------------ | ------------------- | --------------- |
| MPRU01  | Tempo di Risposta | ≤ 10s              | ≤ 5s                | 7s              |
| MPRE01  | Utilizzo Risorse  | ≤ 30%              | ≤ 20%               | 25%             |
| MPRE02  | Capacità          | ≥ 50               | ≥ 100               | 300             |
| MPRM01  | Testabilità       | ≥ 70%              | ≥ 85%               | 100%            |
| MPRP01  | Adattabilità      | ≤ 10m              | ≤ 5min              | 2m              |

Table: Metriche non incluse nel cruscotto
...
| Problema    | Descrizione                                                                                            | Gravità | Soluzione                                                                             |
| ----------- | ------------------------------------------------------------------------------------------------------ | ------- | ------------------------------------------------------------------------------------- |
| Typescript  | La maggior parte dei membri del gruppo non è ancora familiare con questo linguaggio di programmazione. | Bassa   | Ogni membro ha seguito autonomamente un breve corso per acquisire le nozioni di base. |
| API VS Code | La maggior parte dei membri del gruppo non conosce a fondo l'integrazione delle API di VS Code.        | Media   | Consultare la documentazione fornita delle API di VS Code.                            |
| Jest        | La maggior parte dei membri del gruppo non conosce il framework per i test Jest.                       | Media   | Consultare la documentazione fornita da Jest                                          |
