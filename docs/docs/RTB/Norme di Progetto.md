...
Per garantire la standardizzazione nell'utilizzo di %%GitHub|github%% durante il ciclo di sviluppo, vengono definite le seguenti procedure operative per le %%attività|attività%% principali.
...
- **CodiceTicket**: l'identificativo unico del ticket in %%JIRA|jira%% (es. TWD-123).
- **NomeFunzionalità**: una descrizione concisa e leggibile della %%funzionalità|funzionalità%% (es. nuova-feature).
...
Ciascun membro del team deve sviluppare le %%funzionalità|funzionalità%% assegnate in un branch dedicato. Al completamento dello sviluppo, è necessario avviare una Pull Request verso il branch `develop`, utilizzando l'interfaccia web di %%GitHub|github%% o la CLI.

L'apertura di una Pull Request innesca l'esecuzione di una %%GitHub|github%% Action preconfigurata, responsabile dell'esecuzione automatica dei %%test|test%% sul codice proposto. Per completare il merge, è obbligatoria l'approvazione da parte di almeno un _Verificatore_.
...
- **Interfaccia web di %%GitHub|github%%**: Per verificare rapidamente il codice in un ambiente isolato, è possibile avviare un Codespace direttamente dalla PR. Dall'interfaccia web è possibile selezionare la voce: `File Changed > Review in codespaces`.
- **Locale**: Per verificare la Pull request in locale sarà necessario:

  - Accedere alla Pull request tramite %%Github|github%% CLI:
...
- **_Progettista:_** figura professionale incaricata di gestire la progettazione del %%prodotto|prodotto%% software. I suoi incarichi sono:
  ...
  - Prendere scelte riguardanti gli aspetti tecnico/tecnologici per ottenere %%efficacia|efficacia%% ed %%efficienza|efficienza%% massima;
...
Ogni %%sprint|sprint%% avrà durata, per lo meno nella prima parte del %%progetto|progetto%%, di due settimane che poi potrà essere modificata a seconda delle esigenze; sarà suddiviso nelle seguenti fasi:
